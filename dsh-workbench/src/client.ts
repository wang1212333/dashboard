...
function agentPrompt(intent: string, filePath?: string, profile?: DatasetProfile, designTemplate?: DesignStyle, asset?: SemanticAsset): string {
  const verifiedProfile = profile
    ...
    : ''
  const style = designTemplate ? `\n\n已选视觉风格（必须落实到最终 HTML，而不是只在说明中提及）：\n- 模板：${designTemplate.name}（${designTemplate.id}）${designTemplate.description ? `\n- 风格说明：${designTemplate.description}` : ''}${designTemplate.primaryColor ? `\n- 主色：${designTemplate.primaryColor}` : ''}${designTemplate.styleGuide ? `\n- 可执行规则：\n${designTemplate.styleGuide}` : ''}\n- 不使用品牌商标、受版权保护的图片或原站文案；仅使用上述设计语言。` : ''
  if (asset) return `你正在执行一个“受控 AI 看板搭建”任务的 OMD 数据源核验阶段。用户需求：${intent}\n\n用户已在工作台选择资产：\n- 名称：${asset.name}\n- 类型：${asset.entityType}\n- FQN：${asset.fqn}${asset.description ? `\n- 描述：${asset.description}` : ''}${asset.columns.length ? `\n- 已读取字段：${asset.columns.join('、')}` : ''}${asset.glossaryTerms?.length ? `\n- 关联术语：${asset.glossaryTerms.join('、')}` : ''}${style}\n\n这里没有上传数据文件，也没有可用的实际数值。先调用 mcp__openmetadata_semantic__semantic_search（或 search_metadata）复核候选，再对该 FQN 调用 mcp__openmetadata_semantic__get_entity_details，随后调用 workbench_assess_semantic_evidence。只报告可验证的资产、字段、口径、血缘/治理状态和缺口。\n\n严禁调用 workbench_analyze_dataset、workbench_create_dashboard_plan 或 workbench_build_dashboard，也不得生成 SQL、HTML 或任何虚构数值。明确告诉用户：下一步要为这个已核验资产接入只读数据查询 MCP，取得真实数据后才进入“数据画像 → Plan → Confirm → Spec → Draft”。`
  if (!filePath) throw new Error('DATA_SOURCE_REQUIRED')
  return `你正在执行一个“最小 MVP 数据看板搭建”任务。用户需求：${intent}\n\n数据文件（CSV，已在本机保存）：${filePath}${verifiedProfile}${style}\n\n先使用工具读取该文件；不要回显整份 CSV，不要暴露内部推理，也不得直接生成 HTML、JavaScript、SQL 或查询。\n\n只执行以下五步：\n1. 识别用户意图：明确要解决的业务问题、受众和决策。\n2. 理解数据：调用 workbench_analyze_dataset，识别字段、粒度、时间范围、质量风险、推荐模板与字段映射。\n3. 检索语义并与数据验证：OpenMetadata 可用时执行候选检索、详情读取与 workbench_assess_semantic_evidence；不可用或不完整时以数据字段、类型和非空情况验证，并在最终说明中记录限制。\n4. 创建 DashboardPlan：将意图、数据理解、模板、自动字段映射、筛选键、指标和故事线汇总为可读方案；只请用户确认“业务意图与故事线”是否正确，不要求确认字段映射或指标。\n5. 用户确认后调用 workbench_confirm_dashboard_plan({ businessConfirmed: true, storylineConfirmed: true })，再仅用该 Spec 和选定模板调用 workbench_build_dashboard。构建器会从 Plan 绑定的原始 CSV 读取数据，绝不再传入、摘录、重写或采样 CSV；直接返回 HTML、质量报告和 Manifest。\n\n默认信息架构必须服务单一主问题：语境 → 3–6 个核心结论 KPI → 证据支持的主叙事 → 可探索明细 → 数据可信度。故事线由数据决定；不得为了凑版式固定生成模拟、流程或行动页面。\n\n若选中 SKU Operations Workbench，故事线页数为 1–5 页，页面目的与模块组合必须由数据和用户问题决定。storylinePlan 只能编排 Plan 已有的指标和图表；没有数据支持的能力写入 omittedCapabilities，绝不虚构 KPI、阈值、目标、负责人、因果或行动结论。\n\n视觉必须使用确定性 visualContract；若用户未选择风格，使用 universal-default。生成完成即交付，没有版本确认、治理、Preview、Release、发布审批或其他后续工作流。`
}
