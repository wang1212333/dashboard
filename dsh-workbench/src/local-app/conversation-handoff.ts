...
.dashboard-draft-list{display:grid;gap:8px;max-height:184px;overflow-y:auto;margin-bottom:8px;scrollbar-width:thin}
...
section.dashboard-draft-actions button svg{width:15px;height:15px;flex:none;stroke-width:1.6}
section.dashboard-draft-actions button.publish-draft{background:#1F1F1F;border:0;color:#fff;font-weight:500}
section.dashboard-draft-actions{background:rgba(255,255,255,.76)}
.dashboard-draft-actions .draft-copy{overflow:hidden}
.dashboard-draft-actions .draft-info>svg{stroke-width:1.5}
section.dashboard-draft-actions .draft-metadata span{color:#8a8f98}
section.dashboard-draft-actions .draft-buttons button{width:32px;height:32px;padding:0;border:0;background:transparent;color:#343434}
section.dashboard-draft-actions .draft-buttons .publish-draft{background:#1f1f1f;color:#fff}
section.dashboard-draft-actions .draft-buttons button:not(.publish-draft):is(:hover,:focus-visible){background:rgba(31,31,31,.06)}
section.dashboard-draft-actions button:disabled{opacity:.4;cursor:not-allowed}
section.dashboard-draft-actions :focus-visible{outline:2px solid #777;outline-offset:2px}
section.dashboard-draft-actions .draft-info{cursor:pointer;outline-offset:2px}
section.dashboard-draft-actions [aria-busy=true] svg{opacity:.5}
.tooltip[data-draft=true]{height:auto;padding:5px 8px;background:#262626;font-weight:400;box-shadow:0 4px 12px rgba(0,0,0,.12)}
...
export function applyConversationHandoff(page: string): string {
  return page
    ...
    .replace('</body>', `<script>(()=>{function placeDraftCard(){const composer=document.querySelector('.composer-wrapper.chat-composer');if(!composer)return;const cards=document.querySelectorAll('.chat-thread .dashboard-draft-actions');if(!cards.length)return;let list=composer.querySelector('.dashboard-draft-list');if(!list){list=document.createElement('div');list.className='dashboard-draft-list';composer.prepend(list)}list.replaceChildren(...cards)}new MutationObserver(placeDraftCard).observe(document.getElementById('page-content'),{childList:true,subtree:true});placeDraftCard()})()</script></body>`)
    .replaceAll(
    ...
    .replace(
      'function persistActiveHistory(){const index=state.history.findIndex(item=>item.sessionId===state.sessionId);if(index<0)return;state.history[index]={...state.history[index],updatedAt:Date.now(),activityAt:Date.now(),timeline:state.timeline,activity:state.activity,runState:state.runState,stream:state.stream,streamState:state.streamState};saveHistory(state.history)}',
      'function persistActiveHistory(touchActivity=true){const index=state.history.findIndex(item=>item.sessionId===state.sessionId);if(index<0)return;const current=state.history[index],now=Date.now();state.history[index]={...current,updatedAt:now,activityAt:touchActivity?now:(current.activityAt||current.createdAt),timeline:state.timeline,activity:state.activity,runState:state.runState,stream:state.stream,streamState:state.streamState};saveHistory(state.history)}',
    )
    .replace(
    ...
    .replace(
      "persistActiveHistory();render();renderHistory();return}if(data.kind==='history-load-failed'){state.activity=typeof data.message==='string'?data.message:state.activity;refreshStream();return}",
      "persistActiveHistory(false);render();renderHistory();return}if(data.kind==='history-load-failed'){state.activity=typeof data.message==='string'?data.message:state.activity;refreshStream(false);return}",
    )
    .replace(
    ...
    .replace(
      ...
      "const dashboardAgentApi=()=>isHostWorkbench()?'/api/dsh-workbench':'/api';\n  const leakedHttpError=value=>/(?:^|[\\r\\n])\\s*(?:[45]\\d{2}\\s+)?<!doctype\\s+html\\b/i.test(String(value||'').slice(0,2048));\n  const upstreamReplyError='数据已读取，但回答生成服务返回了异常响应。请重试。';\n  function ensureConversationHistory(touchActivity=true){const logicalId=state.sessionId||state.dashboardAgentSessionId;if(!logicalId)return;if(!state.sessionId)state.sessionId=logicalId;const current=state.history.find(item=>item.sessionId===logicalId);const title=current?.title||'生成看板：'+(state.stream?.attachmentName||state.stream?.question?.slice(0,24)||'数据看板');const record={...current,updatedAt:Date.now(),activityAt:touchActivity?Date.now():(current?.activityAt||current?.createdAt||Date.now()),sessionId:logicalId,title,createdAt:current?.createdAt||Date.now(),timeline:state.timeline,activity:state.activity,runState:state.runState||'running',stream:state.stream,streamState:state.streamState,turns:state.conversationTurns,dashboardAgentSessionId:state.dashboardAgentSessionId||logicalId,dashboardAgentUploadId:state.dashboardAgentUploadId};state.history=[record,...state.history.filter(item=>item.sessionId!==logicalId)];saveHistory(state.history);renderHistory()}\n  const isCurrentUpload=",
    )
    .replace(
      "function applyHeadlessAgentEvent(event){",
    ...
    .replaceAll(
      ...
      `for(const draft of s.dashboardDrafts||(s.dashboardDraft?[s.dashboardDraft]:[])){html+='<section class="dashboard-draft-actions"><div class="draft-info" role="button" tabindex="0" aria-label="预览看板" data-preview-draft="'+escapeHtml(draft.assetId)+'" data-draft-revision="'+escapeHtml(draft.revision)+'">${draftIcon('<rect x="3" y="3" width="7" height="9" rx="1"/><rect x="14" y="3" width="7" height="5" rx="1"/><rect x="14" y="12" width="7" height="9" rx="1"/><rect x="3" y="16" width="7" height="5" rx="1"/>')}<div class="draft-copy"><strong>'+escapeHtml(draft.title||'看板草稿')+'</strong><div class="draft-metadata"><span>'+escapeHtml(draft.revision)+'</span><span>'+(draft.released?'已发布':draft.serverPending?'待完成服务器发布':'待发布')+'</span><span class="draft-source-file">'+escapeHtml(s.attachmentName||'')+'</span></div></div></div><div class="draft-buttons"><button type="button" aria-label="预览" data-tooltip="预览" data-preview-draft="'+escapeHtml(draft.assetId)+'" data-draft-revision="'+escapeHtml(draft.revision)+'">${draftIcon('<path d="M2 12s3.5-7 10-7 10 7 10 7-3.5 7-10 7S2 12 2 12Z"/><circle cx="12" cy="12" r="3"/>')}</button><button type="button" class="publish-draft" aria-label="发布看板" data-tooltip="'+(draft.released?'已发布':draft.previewed?'发布看板':'先预览，再确认发布')+'" data-release-draft="'+escapeHtml(draft.assetId)+'" data-draft-revision="'+escapeHtml(draft.revision)+'" '+(draft.released||state.draftActionBusy?'disabled':'')+'>${draftIcon('<path d="M12 16V3m-5 5 5-5 5 5M4 17v4h16v-4"/>')}</button><button type="button" aria-label="分享" data-tooltip="分享" data-share-draft="'+escapeHtml(draft.assetId)+'">${draftIcon('<circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><path d="m8.6 10.5 6.8-4m-6.8 7 6.8 4"/>')}</button><button type="button" aria-label="下载" data-tooltip="下载" data-download-draft="'+escapeHtml(draft.assetId)+'" data-draft-revision="'+escapeHtml(draft.revision)+'">${draftIcon('<path d="M12 3v12m-5-5 5 5 5-5M4 17v4h16v-4"/>')}</button></div></section>'}if(s.error)html+='<p class="agent-block agent-error">'+escapeHtml(s.error)+'</p>';`,
    )
    .replace(
