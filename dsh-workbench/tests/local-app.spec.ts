...
describe('local import app', () => {
  it('moves a submitted prompt and its attachment into the read-only conversation record', () => {
    ...
    expect(page).toContain("url.searchParams.set('section',page)")
  ...
  it('preserves the existing my-dashboards layout while loading the dashboard API', () => {
    ...
    expect(page).toContain('.dash-list>.dash-row[hidden]{display:none!important}')
    expect(page).toContain("querySelector('.dash-grid,.dash-list')")
  ...
  it('keeps the empty-state task launcher centered inside the content column', () => {
    ...
    expect(page).toContain("if(!state.attachOpen)return;if(!document.getElementById('attachment-popover')){state.attachOpen=false;return;}")
    expect(page).toContain("target.closest('#attachment-popover,#upload-button')")
