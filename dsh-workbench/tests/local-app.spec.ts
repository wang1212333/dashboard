...
describe('local import app', () => {
  ...
  it('renders a low-load non-interactive WebGL particle mark behind the empty-state conversation controls', () => {
    const page = renderLocalWorkbenchPage()
    expect(page).toContain('.particle-mark{position:absolute;z-index:0;top:8px;right:-250px')
    expect(page).toContain('pointer-events:none;cursor:default;background:transparent')
    expect(page).not.toContain('.particle-mark::before,.particle-mark::after')
    expect(page).toContain('cursor:default;background:transparent')
    ...
    expect(page).toContain('particleCount=mobile?700:1500')
    ...
    expect(page).not.toContain("panel.addEventListener('pointerdown',scatter)")
    expect(page).not.toContain('velocities=new Float32Array(particleCount*3)')
    expect(page).not.toContain('velocities[i3]*=.88')
