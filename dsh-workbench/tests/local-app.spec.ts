...
describe('local import app', () => {
  it('moves a submitted prompt and its attachment into the read-only conversation record', () => {
    ...
    expect(page).toContain('.dash-card-info{height:116px')
    expect(page).toContain('.dash-card h2{height:40px')
    expect(page).toContain('-webkit-line-clamp:2')
    expect(page).toContain('.dash-meta{height:18px')
    expect(page).toContain('.dash-card-actions{height:30px')
    expect(page).toContain('body:JSON.stringify({prompt,uploadId:state.dashboardAgentUploadId}),signal:state.abortController.signal')
