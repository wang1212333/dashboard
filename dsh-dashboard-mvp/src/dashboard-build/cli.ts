...
if (!inputPath || !outputPath) {
  console.error('Usage: pnpm dashboard:run -- <input.csv> <output.html> [--template content-ops-v1] [--title title] [--spec dashboard-spec.json]')
  process.exitCode = 2
} else {
  ...
  console.log(JSON.stringify({ output: target, validRows: result.quality.validRows, rejectedRows: result.quality.rejectedRows.length, timeframe: result.model.timeframe }, null, 2))
}
